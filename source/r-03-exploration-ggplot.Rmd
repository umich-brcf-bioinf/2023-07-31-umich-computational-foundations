...
> ## Exercise
> 
> How would we modify the above code to display the facets as columns?
> 
> ```{r, 'facet_solution'}
> ggplot(gapminder_1997) + 
>     aes(x = gdpPercap, y = lifeExp) + 
...
> ## Exercise
> 
> After looking at the attributes for `theme()` with `?theme`, how would we change just the size of the plot title?
> 
> ```{r, 'title_size_solution'}
> scatter_plot + theme(plot.title = element_text(size = 16))
> ```
> 
...
> ggplot(data = gapminder_data, aes(x = year, y = gdpPercap, group = country)) +
>     geom_line() + theme_bw() +
>     labs(
>         title = 'GDP per Capita in the Americas 1952 - 2007',
>         x = 'Year', y = 'GDP per Capita')
> ```
> Try to write the code that would generate the following plot:
> 
